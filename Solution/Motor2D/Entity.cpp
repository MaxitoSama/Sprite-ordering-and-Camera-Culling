...
void Entity::Draw(SDL_Texture* sprites, float direction, float dt)
{
	...
	{		
		...
		App->render->FillVec(priority, sprites, position.x, position.y, &(animation->GetCurrentFrame(dt)), 0.2f);
	}
	
	if (type == HOUSE)
	{
		App->render->FillVec(priority,sprites, position.x, position.y,NULL, 0.2);
	}
}
